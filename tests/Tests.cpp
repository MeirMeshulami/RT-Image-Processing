...
TEST(Client, InitalTest) {
	FrameProcessor fp;
	fp.StartServer();
	//std::thread t1([&fp]() {
	//	fp.GetService()->GetFrame();
	//	});

	
	//// service.GetFrame();
	// // Assuming GetFrame() pushed a frame into the frameShowQueue
	Sleep(5000);
	std::shared_ptr<FrameWithDetails> framePtr;
	LOG_INFO(fp.GetService()->frameShowQueue->GetQueueSize());
	fp.GetService()->frameShowQueue->TryPop(framePtr);

	// Check if framePtr is not nullptr before attempting to display
	if (framePtr) {
		cv::imshow("Display Window", framePtr->GetFrame());
		cv::waitKey(0);
	}
	else {
		LOG_INFO("PTR is null");
	}
	//t1.join();
}
